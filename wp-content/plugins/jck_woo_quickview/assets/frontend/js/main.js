...
/* 	=============================
	...
	jQuery(document).ready(function($) {
		...
		if(jckqv.settings.content_ajaxcart == 1) jckqv_setupAddToCart();
		if(jckqv.settings.styling_autohide == 1) jckqv_setupHoverbtn();
	});
...
/* 	=============================
	...
	function jckqv_setupPopup(){
		...
		jQuery('body').on(jckqv.settings.general_method, '*[data-jckqvpid]', function(event){
			...
			var allProds = jckqv_getProducts(),
			    pid = jQuery(this).attr('data-jckqvpid'),
			    popupIndex = jckqv_getArrayIndexForKey(allProds, "pid", pid);
			...
			jQuery.magnificPopup.open({
				...
				gallery: {
					enabled: (jckqv.settings.general_gallery == 1 ? true : false),
					preload: false
				},
				...
				callbacks: {
				    ...
				    elementParse: function( item ) {
    				    ...
    				    this.st.ajax.settings.data = {
                            action: 'jckqv',
                            nonce: jckqv.nonce,
                            pid: this.items[this.index].data.pid
                        }
	...
	function setup_main_slider() {
    	...
    	var fade = (jckqv.settings.imagery_imgtransition == "fade") ? true : false,
    	    autoplay = (jckqv.settings.imagery_autoplay == "1") ? true : false,
    	    infinite = (jckqv.settings.imagery_infinite == "1") ? true : false,
    	    arrows = (jckqv.settings.imagery_navarr == "1") ? true : false,
    	    asNavFor = ( jckqv.settings.imagery_thumbnails == "thumbnails" ) ? '#jckqv_thumbs' : null,
    	    dots = ( jckqv.settings.imagery_thumbnails == "bullets" ) ? true : false;
    	...
    	jQuery('#jckqv_images').on({
			...
			infinite: infinite,
			fade: fade,
			speed: jckqv.settings.imagery_transitionspeed,
			autoplay: autoplay,
			autoplaySpeed: jckqv.settings.imagery_autoplaySpeed,
			arrows: arrows,
			dots: dots
		});
	...
	function setup_thumbnail_slider() {
    	...
    	if( jckqv.settings.imagery_thumbnails == "thumbnails" ) {
	...
	function jckqv_setupAddToCart(){
		jQuery("body").on('submit', "#jckqv form.cart", function(e){
	        ...
	        jckqv_addToCart(query, $the_buton, function(response){
		    	...
		    	if(jckqv.settings.content_autohidepopup == 1 && response.result != "fail")
		    	{
		    		setTimeout(function(){
   	...
   	function jckqv_setupHoverbtn() {
	   	jQuery('body').on({
		    mouseenter: function () {
		    	...
		    	if(jQuery(this).find(jckqv.settings.styling_hoverel).length <= 0){
		...
		}, jckqv.settings.styling_hoverel );
   	}
...
/* 	=============================
   	 ...
   	 * @return index
   	 */
	function jckqv_getArrayIndexForKey(arr, key, pid){
